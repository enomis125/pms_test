  ...
  return (
    <main>
      <div className="flex flex-col mt-3 py-3">
        <p className="text-xs px-6">{t("hotel.tipologiesGroup.label")}</p>
        <div className="flex flex-row justify-between items-center mx-5">
          ...
          <TipologyGroupForm
            ...
            buttonColor={"primary"}
            modalHeader={t("hotel.tipologiesGroup.new.modalHeader")}
            modalIcons={"bg-red"}
            formTypeModal={11}
          ></TipologyGroupForm>
        </div>
            ...
            >
              <TableHeader>
                <TableColumn className="bg-primary-600 text-white font-bold w-[40px] uppercase">
                  {t("hotel.tipologiesGroup.datatable.id")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold  uppercase">
                  {t("hotel.tipologiesGroup.datatable.code")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold uppercase">
                  {t("hotel.tipologiesGroup.datatable.shortname")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold w-1/4 px-10 uppercase">
                  {t("hotel.tipologiesGroup.datatable.description")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold uppercase">
                  {t("hotel.tipologiesGroup.datatable.details")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold uppercase">
                  {t("hotel.tipologiesGroup.datatable.state")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold uppercase">
                  {t("hotel.tipologiesGroup.datatable.order")}
                </TableColumn>
                ...
                {items.map((roomtypesgroups, index) => (
                  <TableRow key={index}>
                    <TableCell className="text-left underline text-blue-600"><TipologyGroupForm
                      ...
                      modalHeader={t("hotel.tipologiesGroup.edit.modalHeader")}
                    ...
                    <TableCell className="flex justify-end">
                      <Dropdown>
                        ...
                        <DropdownMenu aria-label="Static Actions" closeOnSelect={false} isOpen={true}>
                          ...
                          <DropdownItem key="delete" onClick={() => handleDelete(roomtypesgroups.roomTypeGroupID)}>{t("general.removeRecord")}</DropdownItem>
