...
export default function Marketing() {
  ...
  const handleDelete = async (idMarketing) => {
    try {
      ...
      alert(t("cardex.marketing.deleteSuccess"));
    } catch (error) {
      console.error(t("cardex.marketing.deleteError"), error.message);
    }
  };
  ...
  return (
    <main>
      <div className="flex flex-col mt-3 py-3">
        <p className="text-xs px-6">{t('cardex.marketing.marketingHeader')}</p>
        <div className="flex flex-row justify-between items-center mx-5">
          <div className="flex flex-row">
            <div className="flex flex-wrap md:flex-nowrap gap-4">
              <Input
                className="mt-4 w-80"
                placeholder={t("cardex.marketing.searchPlaceholder")}
          ...
          <MarketingForm
            buttonName={t("cardex.marketing.buttonNew")}
            buttonIcon={<FiPlus size={15} />}
            buttonColor={"primary"}
            modalHeader={t("cardex.marketing.modalInsertHeader")}
            modalIcons={"bg-red"}
            formTypeModal={11}
          ></MarketingForm>
        </div>
            ...
            >
              <TableHeader>
                <TableColumn className="bg-primary-600 text-white font-bold w-[40px] uppercase">
                  {t("cardex.marketing.headerID")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold uppercase">
                  {t("cardex.marketing.headerGroup")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold w-1/4 px-10 uppercase">
                  {t("cardex.marketing.headerAbbreviation")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold uppercase">
                  {t("cardex.marketing.headerDescription")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold w-1/4 px-10 uppercase">
                  {t("cardex.marketing.headerOrder")}
                </TableColumn>
                <TableColumn className="bg-primary-600 text-white font-bold w-1/4 px-10 uppercase">
                  {t("cardex.marketing.headerProperty")}
                </TableColumn>
                  ...
                  <TableRow key={index}>
                    <TableCell className="text-left underline text-blue-600">
                      <MarketingForm
                        ...
                        modalHeader={t("cardex.marketing.modalEditHeader")}
                        modalEditArrow={<BsArrowRight size={25} />}
                        modalEdit={`${t("cardex.marketing.modalEditID")}${marketing.marketingID}`}
                        formTypeModal={12}
