    ...
    useEffect(() => {
        const getData = async () => {
            try {
                ...
                const filteredData = res.data.response.filter(tipologyGroup => tipologyGroup.label !== "");
...
>
  {tipologyGroup.map((tipologyGroup) => (
    <AutocompleteItem
      ...
      value={tipologyGroup}
      textValue={tipologyGroup.roomTypeGroupID.toString()} // Provide a textValue prop
